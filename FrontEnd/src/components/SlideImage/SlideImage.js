...
import {next_icon_1, previous_icon_1, about_art_main} from "~/assets/img";
...
function SlideImage({slideImageList, setChoosingImage, choosingImage}) {
    ...
    const slideImageTemplate = slideImage => {
        var isSelected = choosingImage === slideImage?.images?.imageLink;
        return (
            <>
                <div style={{width: "100%"}} className={clsx(styles["about-card--carousel"], { [styles["selected"]]: isSelected })}>
                    <img
                        ...
                        className={clsx(styles['about-card__img'])} src={about_art_main} alt=""/>
                </div>
            </>
        )
    };
